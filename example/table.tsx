            ...
            <div className="row header">
                {columns.map(c => (
                    <div
                        ...
                        {c.width}
                        <span onClick={() => onSortBy(c.key)}>
                            {c.key} / {c.accessorKey} {isSortedBy(sortedBy, c.key) > 0 ? (isSortedBy(sortedBy, c.key) === 1 ? `Up` : `Down`) : `Sort`}
                        </span>
                        <input
                            type="text"
                            style={{width: '100%'}}
                            value={queryParts[c.key]}
                            onChange={e => {
                                const v = e.target.value
                                setQueryParts(x => ({...x, [c.key]: v}))
                            }}
                        />
                        <div
